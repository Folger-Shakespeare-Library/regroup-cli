...
var listCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		resp, err := client.ListGroups(api.ListGroupsParams{
			Name:     listFlagName,
			Page:     listFlagPage,
			Count:    listFlagCount,
		...
		case "table":
			...
			fmt.Fprintln(w, "ID\tNAME\tDESCRIPTION\tCONTACTS\tMEMBERS\tACCESS")
			for _, g := range resp.Results {
				fmt.Fprintf(w, "%d\t%s\t%s\t%d\t%d\t%s\n",
					g.ID, g.Name, g.Description, g.Contacts, g.Members, g.Access)
			}
			return w.Flush()
		default:
...
func init() {
	...
	listCmd.Flags().StringVar(&listFlagSortBy, "sort-by", "", "Sort field (name, description, category, access, contacts, members)")
	listCmd.Flags().StringVar(&listFlagSortType, "sort-type", "", "Sort direction (ASC, DESC)")
	listCmd.Flags().BoolVar(&listFlagAll, "all", true, "Fetch all groups (use --all=false to paginate)")
